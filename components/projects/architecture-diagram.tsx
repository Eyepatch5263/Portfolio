...
export const projectArchitecture: Record<string, { nodes: ArchitectureNode[]; connections: ArchitectureConnection[] }> = {
    "explain-bytes": {
        nodes: [
            ...
            { id: "fs", label: "Node FS", type: "database" as const, x: 80, y: 35, description: "Local file system serving as the source of truth for MDX documentation and flashcards." },
            { id: "resend", label: "Resend", type: "external" as const, x: 80, y: 65, description: "Email service for verification emails and welcome messages." },
        ],
        connections: [
            ...
            { from: "api", to: "resend", label: "Emails" },
        ],
    },
    "collaborative-editor": {
        nodes: [
            ...
            { id: "postgres", label: "Postgres DB", type: "database" as const, x: 55, y: 50, description: "Primary storage for document states in JSON and MDX formats with conflict resolution." },
        ...
        connections: [
            ...
            { from: "api", to: "postgres", label: "Persistence" },
            { from: "postgres", to: "github", label: "Admin Push" },
            { from: "github", to: "fs", label: "I/O & Deploy", animated: true },
        ],
    },
    "anima": {
        nodes: [
            { id: "client", label: "Next.js Frontend", type: "frontend" as const, x: 10, y: 50, description: "User interface for intent-based searches, displaying results, and filtering anime/manga lists." },
            { id: "api", label: "Next.js API", type: "backend" as const, x: 35, y: 50, description: "Serverless backend routes orchestrating the semantic search pipeline, querying Qdrant, and querying Anilist GraphQL." },
            { id: "anilist", label: "Anilist GraphQL", type: "external" as const, x: 35, y: 15, description: "External API for querying anime/manga details like trending, popular this season, and upcoming." },
            { id: "postgres", label: "Postgres DB", type: "database" as const, x: 60, y: 85, description: "Postgres database holding raw metadata for 14,000 anime before vector processing." },
            { id: "qdrant", label: "Qdrant Vector DB", type: "database" as const, x: 75, y: 50, description: "Vector database storing 384-dimensional multi-vector embeddings (archetypes, synopsis, atmosphere, themes) for dense retrieval." },
            { id: "cloud_ai", label: "Cloud AI Services", type: "backend" as const, x: 85, y: 15, description: "Docker containers hosting Ollama (BGE-small, Qwen2.5), cross-encoding models, re-ranking models, and MMR diversity scoring." },
        ],
        connections: [
            { from: "client", to: "api", label: "Search / Query", animated: true },
            { from: "api", to: "anilist", label: "Metadata Fetch" },
            { from: "api", to: "qdrant", label: "Dense Retrieval", animated: true },
            { from: "api", to: "cloud_ai", label: "Re-rank / Embed", animated: true },
            { from: "postgres", to: "cloud_ai", label: "Pipeline Feed" },
            { from: "cloud_ai", to: "qdrant", label: "Upsert Vectors" },
        ],
    },
    "onlyanime": {
        nodes: [
